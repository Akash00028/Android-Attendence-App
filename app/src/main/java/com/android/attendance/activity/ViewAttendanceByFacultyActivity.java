...
public class ViewAttendanceByFacultyActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		attendanceList.add("Id | StudentName      |  Status  | reg no.");
		...
		for(AttendanceBean attendanceBean : attendanceBeanList)
		{
			...
			if(attendanceBean.getAttendance_session_id() != 0)
			{
				...
				users = attendanceBean.getAttendance_student_id()+".     "+studentBean.getStudent_firstname()+" "+studentBean.getStudent_lastname()+"\t  "+attendanceBean.getAttendance_status()+"  \t"+studentBean.getStudent_address();
			}
			else
			{
